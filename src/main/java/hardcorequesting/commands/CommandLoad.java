...
public class CommandLoad extends CommandBase {
	...
	private static final FileFilter QUEST_SET_FILTER = new FileFilter() {
		...
		public boolean accept(File pathname) {
			return JSON.matcher(pathname.getName()).find() && !REPUTATIONS.matcher(pathname.getName()).find()
					&& !BAGS.matcher(pathname.getName()).find();
		}
	};

	public CommandLoad() { super("load", "all"); }
	...
	private void loadSet(ICommandSender sender, File file) {
		if (!file.exists()) { throw new CommandException(Lang.FILE_NOT_FOUND); }
		try {
			...
			if (set != null) {
				sender.addChatMessage(new ChatComponentText(
						StatCollector.translateToLocalFormatted(Lang.LOAD_SUCCESS, set.getName())));
			}
			else {
