...
public class CommandHelp extends CommandBase {
		...
		switch (arguments.length) {
			case 0:
				...
				for (int i = 0; i < commands.size() - 1; i++) {
					output.append("/").append(CommandHandler.instance.getCommandName()).append(" ").append(YELLOW)
							.append(commands.get(i)).append(WHITE).append(", ");
				}
				output.delete(output.length() - 2, output.length());
				output.append(" and /").append(CommandHandler.instance.getCommandName()).append(" ").append(YELLOW)
						.append(commands.get(commands.size() - 1)).append(WHITE).append(".");
				sender.addChatMessage(new ChatComponentText(output.toString()));
				break;
			case 1:
				String commandName = arguments[0];

				if (!CommandHandler.commandExists(commandName)) { throw new CommandNotFoundException(Lang.NOT_FOUND); }
				ISubCommand command = CommandHandler.getCommand(commandName);
				if (command.isVisible(sender)) {
					for (int i : command.getSyntaxOptions(sender))
						sender.addChatMessage(new ChatComponentText(YELLOW
								+ Translator.translate(Lang.COMMAND_PREFIX + commandName + Lang.SYNTAX_SUFFIX + i)
								+ WHITE + " - "
	...
	@Override
	public boolean isVisible(ICommandSender sender) { return true; }
