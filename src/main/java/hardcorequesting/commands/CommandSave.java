...
public class CommandSave extends CommandBase {
	...
	public CommandSave() { super("save", "all", "bags"); }

	@Override
	public void handleCommand(ICommandSender sender, String[] arguments) {
		if (arguments.length == 1 && arguments[0].equals("all")) {
			try {
				save(sender, Reputation.getReputationList(), new TypeToken<List<Reputation>>() {}.getType(),
						"reputations");
	...
	public List<String> addTabCompletionOptions(ICommandSender sender, String[] args) {
		...
		for (QuestSet set : Quest.getQuestSets()) {
			if (pattern.matcher(set.getName()).find())
	...
	private static boolean stringsMatch(String[] sub, String[] search) {
		for (int i = 0; i < sub.length; i++) {
			if (!sub[i].equalsIgnoreCase(search[i]))
				return false;
		}
		return true;
	}
	...
	private static void save(ICommandSender sender, Object save, Type type, String name) {
		try {
			...
			fileWriter.close();
			sender.addChatMessage(new ChatComponentText(StatCollector.translateToLocalFormatted(Lang.SAVE_SUCCESS,
					file.getPath().substring(HardcoreQuesting.configDir.getParentFile().getParent().length()))));
		}
		catch (IOException e) {
	...
	@Override
	public int[] getSyntaxOptions(ICommandSender sender) { return new int[] { 0, 1, 2 }; }
}
