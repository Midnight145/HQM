...
public abstract class CommandBase implements ISubCommand {
	...
	static {
		GSON = new GsonBuilder()
				.registerTypeAdapter(Reputation.getReputationList().getClass(), ReputationAdapter.REPUTATION_ADAPTER)
				.registerTypeAdapter(QuestSet.class, QuestAdapter.QUEST_SET_ADAPTER)
				.registerTypeAdapter(GroupTier.class, BagAdapter.GROUP_TIER_ADAPTER).setPrettyPrinting()
	...
	@Override
	public boolean isVisible(ICommandSender sender) { return getPermissionLevel() <= 0 || isPlayerOp(sender); }

	@Override
	public int[] getSyntaxOptions(ICommandSender sender) { return new int[] { 0 }; }
	...
	protected void sendChat(ICommandSender sender, String key, Object... args) { sendChat(sender, false, key, args); }
	...
	protected boolean isPlayerOp(ICommandSender sender) { return CommandHandler.isOwnerOrOp(sender); }
