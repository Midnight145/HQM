...
public class QuestData {
	...
	public QuestData(int players) { reward = new boolean[players]; }
	...
	private int getId(EntityPlayer player) {
		...
		for (Team.PlayerEntry entry : team.getPlayers()) {
			if (entry.isInTeam()) {
				if (entry.getName().equals(QuestingData.getUserName(player))) { return id; }
				id++;
			}
		}
	...
	public boolean canClaim() { return completed && !claimed; }
}
