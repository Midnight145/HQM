...
public abstract class QuestTaskItems extends QuestTask {
	...
	public ItemRequirement[] getItems() { return items; }
	...
	public static class ItemRequirement {
		...
		public ItemPrecision getPrecision() { return precision; }
	...
	public void draw(GuiQuestBook gui, EntityPlayer player, int mX, int mY) {
		...
		for (int i = 0; i < items.length; i++) {
			...
			float textSize = 0.8F;
			gui.drawStringWithShadow(str, (int) (item.x + SIZE - gui.getStringWidth(str) * textSize),
					(int) (item.y + SIZE - TEXT_HEIGHT * textSize + 2), textSize,
			...
			for (int i = 0; i < items.length; i++) {
				...
				if (gui.inBounds(item.x, item.y, SIZE, SIZE, mX, mY)) {
					if (isOpBookWithShiftKeyDown) {
						...
						gui.setEditMenu(new GuiEditMenuItem(gui, player,
								item.hasItem ? item.item != null ? item.item.copy() : null : item.fluid, i,
								getMenuTypeId(), item.required, item.precision));
					}
	...
	@Override
	public Class<? extends QuestDataTask> getDataType() { return QuestDataTaskItems.class; }
