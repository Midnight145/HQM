...
public class QuestTaskMob extends QuestTask {
	...
	public void onLivingDeath(LivingDeathEvent event) {
		...
		if (player != null && parent.isEnabled(player) && parent.isAvailable(player) && this.isVisible(player)
				&& !isCompleted(player)) {
	...
	public void draw(GuiQuestBook gui, EntityPlayer player, int mX, int mY) {
		...
		for (int i = 0; i < mobs.length; i++) {
			...
			if (killed == mob.count) {
				gui.drawString(GuiColor.GREEN + Translator.translate("hqm.mobTask.allKilled"),
						x + X_TEXT_OFFSET + X_TEXT_INDENT, y + Y_TEXT_OFFSET + 9, 0.7F, 0x404040);
			}
			else {
				gui.drawString(Translator.translate("hqm.mobTask.partKills", killed, (100 * killed / mob.count)),
						x + X_TEXT_OFFSET + X_TEXT_INDENT, y + Y_TEXT_OFFSET + 9, 0.7F, 0x404040);
			}
			gui.drawString(Translator.translate("hqm.mobTask.totalKills", mob.count), x + X_TEXT_OFFSET + X_TEXT_INDENT,
					y + Y_TEXT_OFFSET + 15, 0.7F, 0x404040);
		}
	}
				...
				if (gui.inBounds(x, y, ITEM_SIZE, ITEM_SIZE, mX, mY)) {
					switch (gui.getCurrentMode()) {
						case MOB:
							...
							break;
						case ITEM:
							gui.setEditMenu(new GuiEditMenuItem(gui, player, mob.icon, i, GuiEditMenuItem.Type.MOB, 1,
	...
	@Override
	public Class<? extends QuestDataTask> getDataType() { return QuestDataTaskMob.class; }
