...
public class QuestSet {
	...
	public String getName(int i) { return i + 1 + ". " + this.name; }
	...
	private boolean isEnabled(EntityPlayer player, Map<Quest, Boolean> isVisibleCache,
		...
		if (this.quests.isEmpty()) { return false; }

		for (final Quest quest : this.quests) {
			if (quest.isEnabled(player, isVisibleCache, isLinkFreeCache)) { return true; }
		}
	...
	public boolean isCompleted(EntityPlayer player) {
		...
		for (final Quest quest : this.quests) {
			if (!quest.isCompleted(player)) { return false; }
		}

		return true;
	}

	public void removeQuest(Quest quest) { this.quests.remove(quest); }

	public void addQuest(Quest quest) { this.quests.add(quest); }

	public void removeRepBar(ReputationBar repBar) { this.reputationBars.remove(repBar); }
	...
	public static void drawOverview(GuiQuestBook gui, ScrollBar setScroll, ScrollBar descriptionScroll, int x, int y) {
		...
		final int start = setScroll.isVisible(gui)
				? Math.round((Quest.getQuestSets().size() - GuiQuestBook.VISIBLE_SETS) * setScroll.getScroll())
				: 0;
		...
		for (int i = start; i < Math.min(start + GuiQuestBook.VISIBLE_SETS, questSets.size()); i++) {
			...
			final int total = questSet.getQuests().size();
			...
			final boolean inBounds = gui.inBounds(GuiQuestBook.LIST_X, setY, gui.getStringWidth(questSet.getName(i)),
					GuiQuestBook.TEXT_HEIGHT, x, y);
		...
		if (Quest.isEditing && gui.getCurrentMode() == EditMode.CREATE) {
			gui.drawString(gui.getLinesFromText(Translator.translate("hqm.questBook.createNewSet"), 0.7F, 130),
					GuiQuestBook.DESCRIPTION_X, GuiQuestBook.DESCRIPTION_Y, 0.7F, 0x404040);
		}
		else {
			if (GuiQuestBook.selectedSet != null) {
				final int startLine = descriptionScroll.isVisible(gui) ? Math.round(
						(GuiQuestBook.selectedSet.getDescription(gui).size() - GuiQuestBook.VISIBLE_DESCRIPTION_LINES)
								* descriptionScroll.getScroll())
						: 0;
				gui.drawString(GuiQuestBook.selectedSet.getDescription(gui), startLine,
						GuiQuestBook.VISIBLE_DESCRIPTION_LINES, GuiQuestBook.DESCRIPTION_X, GuiQuestBook.DESCRIPTION_Y,
						0.7F, 0x404040);
			}

			drawQuestInfo(gui, GuiQuestBook.selectedSet, GuiQuestBook.DESCRIPTION_X,
					GuiQuestBook.selectedSet == null ? GuiQuestBook.DESCRIPTION_Y : INFO_Y, isVisibleCache,
					isLinkFreeCache);
		}
	...
	public void draw(GuiQuestBook gui, int x0, int y0, int x, int y) {
		if (gui.isOpBook) {
			gui.drawString(gui.getLinesFromText(Translator.translate("hqm.questBook.shiftSetReset"), 0.7F, 130), 184,
					192, 0.7F, 0x707070);
		}
				...
				for (final Quest parent : child.getRequirement()) {
					if ((Quest.isEditing || parent.isVisible(player, isVisibleCache, isLinkFreeCache))
						...
						gui.drawLine(gui.getLeft() + parent.getGuiCenterX(), gui.getTop() + parent.getGuiCenterY(),
								gui.getLeft() + child.getGuiCenterX(), gui.getTop() + child.getGuiCenterY(), 5, color);
					}
				}
			}
		}
		...
		for (final Quest quest : this.getQuests()) {
			if (Quest.isEditing || quest.isVisible(player, isVisibleCache, isLinkFreeCache)) {
				...
				gui.drawRect(quest.getGuiX(), quest.getGuiY(), quest.getGuiU(), quest.getGuiV(player, x, y),
						quest.getGuiW(), quest.getGuiH());
			...
			if ((editing || quest.isVisible(player, isVisibleCache, isLinkFreeCache)) && quest.isMouseInObject(x, y)) {
				...
				if (!enabled || editing) {
					...
					if (editing && totalParentCount > 0) {
						txt += "\n" + GuiColor.GRAY + Translator.translate(totalParentCount != 1,
								"hqm.questBook.parentCount", totalParentCount - totalCompletedCount, totalParentCount);
					...
					final int allowedUncompleted = quest.getUseModifiedParentRequirement()
							? Math.max(0, quest.getRequirement().size() - quest.getParentRequirementCount())
							: 0;
					if (parentCount - completed > allowedUncompleted || editing && parentCount > 0) {
						txt += "\n" + GuiColor.PINK
								+ Translator.translate(totalParentCount != 1, "hqm.questBook.parentCountElsewhere",
										totalParentCount - totalCompletedCount, totalParentCount);
						shouldDrawText = true;
						if (editing) {
							if (Keyboard.isKeyDown(Keyboard.KEY_E)) {
								txt += " [" + Translator.translate("hqm.questBook.holding", "E") + "]";
								for (final Quest parent : externalQuests) {
									txt += "\n" + GuiColor.PINK + parent.getName() + " ("
											+ parent.getQuestSet().getName() + ")";
									if (parent.isCompleted(player)) {
										txt += " " + GuiColor.WHITE + " ["
				...
				if (enabled || editing) {
					...
					final String repeatMessage = enabled ? quest.getRepeatInfo().getMessage(quest, player)
					...
					if (editing) {
						...
						final String triggerMessage = quest.getTriggerType().getMessage(quest);
							...
							if (quest.isLinkFree(player, isLinkFreeCache)) {
								...
								switch (quest.getTriggerType()) {
									...
									case QUEST_TRIGGER:
										...
										break;
									case TASK_TRIGGER:
										invisibilityMessage = Translator.translate(quest.getTriggerTasks() != 1,
												"hqm.questBook.invisCount", quest.getTriggerTasks());
										break;
									default:
								...
								if (parentInvisible) {
									...
									if (invisibilityMessage == null) {
										...
										invisibilityMessage = parentText + " "
												+ Translator.translate("hqm.questBook.and") + " " + invisibilityMessage;
									}
								}
	...
	public static void mouseClickedOverview(GuiQuestBook gui, ScrollBar setScroll, int x, int y) {
		...
		for (int i = start; i < Math.min(start + GuiQuestBook.VISIBLE_SETS, questSets.size()); i++) {
			...
			final int setY = GuiQuestBook.LIST_Y + (i - start) * (GuiQuestBook.TEXT_HEIGHT + GuiQuestBook.TEXT_SPACING);
			if (gui.inBounds(GuiQuestBook.LIST_X, setY, gui.getStringWidth(questSet.getName(i)),
					GuiQuestBook.TEXT_HEIGHT, x, y)) {
		...
		if ((Quest.isEditing && gui.getCurrentMode() == EditMode.RENAME)
				&& gui.inBounds(GuiQuestBook.DESCRIPTION_X, GuiQuestBook.DESCRIPTION_Y, 130,
						(int) (GuiQuestBook.VISIBLE_DESCRIPTION_LINES * GuiQuestBook.TEXT_HEIGHT * 0.7F), x, y)) {
			...
			switch (gui.getCurrentMode()) {
				case CREATE:
					if (x > 0 && Quest.size() < DataBitHelper.QUESTS.getMaximum()) {
						int i = 0;
						for (final Quest quest : this.getQuests()) {
							if (quest.getName().startsWith("Unnamed")) {
								i++;
							}
						}
						final Quest newQuest = new Quest(Quest.size(), "Unnamed" + (i == 0 ? "" : i), "Unnamed quest",
								0, 0, false);
					...
					if (Quest.isEditing && gui.getCurrentMode() != EditMode.NORMAL) {
						switch (gui.getCurrentMode()) {
							...
							case SIZE:
								...
								break;
							case ITEM:
								gui.setEditMenu(new GuiEditMenuItem(gui, player, quest.getIcon(), quest.getId(),
										GuiEditMenuItem.Type.QUEST_ICON, 1, ItemPrecision.PRECISE));
								break;
							case DELETE:
