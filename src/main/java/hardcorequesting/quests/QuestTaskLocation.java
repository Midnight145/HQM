...
public class QuestTaskLocation extends QuestTask {
			...
			if (!world.isRemote) {
				...
				for (int i = 0; i < locations.length; ++i) {
					...
					if (!visited[i] && player.worldObj.provider.dimensionId == location.dimension) {
						int current = (int) player.getDistanceSq(location.x + 0.5D, location.y + 0.5D,
								location.z + 0.5D);
						...
						if (location.radius >= 0 && current > target) {
							...
							if (!this.isCompleted(player) && this.isVisible(player) && this.parent.isEnabled(player)
	...
	public static class Location {
		...
		public Visibility getVisible() { return visible; }

		public void setVisible(Visibility visible) { this.visible = visible; }
	...
	public enum Visibility {
		FULL("Full", true, true), LOCATION("Location", true, false), NONE("None", false, false);

		private boolean showCoordinate;
		private boolean showRadius;
		private String id;

		public boolean doShowCoordinate() { return showCoordinate; }

		public boolean doShowRadius() { return showRadius; }
		...
		public String getName() { return Translator.translate("hqm.locationMenu.vis" + id + ".title"); }

		public String getDescription() { return Translator.translate("hqm.locationMenu.vis" + id + ".desc"); }
	}
	...
	public void draw(GuiQuestBook gui, EntityPlayer player, int mX, int mY) {
		...
		for (int i = 0; i < locations.length; i++) {
			...
			if (visited(i, player)) {
				gui.drawString(GuiColor.GREEN + Translator.translate("hqm.locationMenu.visited"),
						x + X_TEXT_OFFSET + X_TEXT_INDENT, y + Y_TEXT_OFFSET + 9, 0.7F, 0x404040);
			}
			else if (location.visible.doShowCoordinate()) {
				if (location.radius >= 0) {
					gui.drawString("(" + location.x + ", " + location.y + ", " + location.z + ")",
							x + X_TEXT_OFFSET + X_TEXT_INDENT, y + Y_TEXT_OFFSET + 9, 0.7F, 0x404040);
				}
				...
				if (player.worldObj.provider.dimensionId == location.dimension) {
					...
					gui.drawString(Translator.translate("hqm.locationMenu.wrongDim"), x + X_TEXT_OFFSET + X_TEXT_INDENT,
							y + Y_TEXT_OFFSET + (location.radius >= 0 ? 15 : 9), 0.7F, 0x404040);
				}
				...
				if (gui.inBounds(x, y, ITEM_SIZE, ITEM_SIZE, mX, mY)) {
					switch (gui.getCurrentMode()) {
						case LOCATION:
							...
							break;
						case ITEM:
							gui.setEditMenu(new GuiEditMenuItem(gui, player, location.icon, i,
									GuiEditMenuItem.Type.LOCATION, 1, ItemPrecision.PRECISE));
							break;
						case RENAME:
	...
	@Override
	public Class<? extends QuestDataTask> getDataType() { return QuestDataTaskLocation.class; }
