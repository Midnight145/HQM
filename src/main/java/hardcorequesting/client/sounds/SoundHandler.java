...
public class SoundHandler {
	...
	public static boolean loadLoreReading(String path) {
		...
		if (index == -1) {
			if (new File(path + "lore.ogg").exists()) {
				...
				Map resourceManagers = ReflectionHelper.getPrivateValue(SimpleReloadableResourceManager.class,
						(SimpleReloadableResourceManager) Minecraft.getMinecraft().getResourceManager(), 2);
				...
				Method method = ReflectionHelper.findMethod(net.minecraft.client.audio.SoundHandler.class, handler,
						new String[] { "loadSoundResource", "func_147693_a", "a" }, ResourceLocation.class,
						SoundList.class);
				if (method == null || handler == null) { return false; }
				try {
	...
	@SideOnly(Side.CLIENT)
	public static void playLoreMusic() { loreSound = play(LABEL + loreNumber, 4F, 1F); }
	...
	public static boolean hasLoreMusic() { return loreMusic; }
