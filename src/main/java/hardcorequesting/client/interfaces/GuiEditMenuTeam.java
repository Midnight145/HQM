...
public class GuiEditMenuTeam extends GuiEditMenu {
	...
	public GuiEditMenuTeam(GuiQuestBook gui, EntityPlayer player) {
		...
		buttons.add(new LargeButton("hqm.party.create", 250, 20) {
			@Override
			public boolean isEnabled(GuiBase gui, EntityPlayer player) { return teamName.getText().length() > 0; }
			...
			@Override
			public void onClick(GuiBase gui, EntityPlayer player) { getTeam().create(teamName.getText()); }
		});

		buttons.add(inviteButton = new LargeButton("hqm.party.invitePlayer", 250, 20) {
			@Override
			public boolean isEnabled(GuiBase gui, EntityPlayer player) { return inviteName.getText().length() > 0; }
			...
			@Override
			public void onClick(GuiBase gui, EntityPlayer player) { getTeam().invite(inviteName.getText()); }
		});
		...
		buttons.add(new LargeButton("hqm.party.accept", 180, 20) {
			...
			@Override
			public boolean isVisible(GuiBase gui, EntityPlayer player) { return inviteTeam != null; }

			@Override
			public void onClick(GuiBase gui, EntityPlayer player) { inviteTeam.accept(); }
		});
		...
		buttons.add(new LargeButton("hqm.party.decideLater", 180, 40) {
			...
			@Override
			public void onClick(GuiBase gui, EntityPlayer player) { inviteTeam = null; }
		});

		buttons.add(new LargeButton(null, 250, 50) {
			@Override
			public boolean isEnabled(GuiBase gui, EntityPlayer player) { return !selectedEntry.isOwner(); }
		...
		buttons.add(new LargeButton("hqm.party.leave", 250, 160) {
			@Override
			public boolean isEnabled(GuiBase gui, EntityPlayer player) { return GuiScreen.isShiftKeyDown(); }
			...
			@Override
			public void onClick(GuiBase gui, EntityPlayer player) { getTeam().leave(); }
		});
		...
		buttons.add(new LargeButton("hqm.party.list", 250, 190) {
			...
			@Override
			public boolean isVisible(GuiBase gui, EntityPlayer player) { return true; }
		...
		textBoxes.add(teamName = new TextBoxName(gui, "", 180, 26) {
			@Override
			protected boolean isVisible() { return getTeam().isSingle() && inviteTeam == null; }
		});
		teamName.setWidth((int) ((GuiQuestBook.PAGE_WIDTH - TITLE_X - 10) * 0.7F));

		textBoxes.add(inviteName = new TextBoxName(gui, "", 180, 26) {
			@Override
			protected boolean isVisible() { return !getTeam().isSingle() && getEntry(getTeam()).isOwner(); }
		});
		...
		scrollBars.add(inviteScroll = new ScrollBar(155, 22, 186, 171, 69, PLAYER_X) {
			...
			public boolean isVisible(GuiBase gui) {
				return inviteTeam == null && getTeam().isSingle() && getTeam().getInvites() != null
						&& getTeam().getInvites().size() > VISIBLE_INVITES;
			}
		});
		...
		scrollBars.add(memberScroll = new ScrollBar(155, 22, 186, 171, 69, PLAYER_X) {
			@Override
			public boolean isVisible(GuiBase gui) {
				return (inviteTeam != null && inviteTeam.getPlayers().size() > VISIBLE_MEMBERS)
						|| (!getTeam().isSingle() && getTeam().getPlayers().size() > VISIBLE_MEMBERS);
			}
		});
	}
	...
	public void draw(GuiBase gui, int mX, int mY) {
		...
		if (team.isSingle() && inviteTeam == null) {
			...
			if (inviteCount > 0) {
				...
				int start = inviteScroll.isVisible(gui)
						? Math.round((team.getInvites().size() - VISIBLE_INVITES) * inviteScroll.getScroll())
						: 0;
			...
			int y = 0;
			int start = memberScroll.isVisible(gui)
					? Math.round(((isOwner ? players.size() : (inviteTeam == null ? team : inviteTeam).getPlayerCount())
			...
			for (Team.PlayerEntry player : players) {
				...
				if (y >= start) {
					...
					if (isOwner) {
						...
						else if (gui.inBounds(PLAYER_X, PLAYER_Y + PLAYER_SPACING * (y - start),
								(int) (gui.getStringWidth(player.getName()) * 0.7F), (int) (TEXT_HEIGHT * 0.7F), mX,
								mY)) {
							color = 0x808080;
						}
					}
				...
				if (entry.isOwner()) {
					...
					if (selectedEntry != null) {
						gui.drawString(gui.getLinesFromText(
								Translator.translate("hqm.party.currentSelection", selectedEntry.getName()), 0.7F, 70),
								177, 52, 0.7F, 0x404040);

						if (selectedEntry.isOwner()) {
							gui.drawString(
									gui.getLinesFromText(Translator.translate("hqm.party.shiftCtrlConfirm"), 0.6F, 70),
									177, 162, 0.6F, GuiColor.RED.getHexColor());
						}
					}

				}
				else {
					gui.drawString(gui.getLinesFromText(Translator.translate("hqm.party.shiftConfirm"), 0.7F, 70), 177,
							162, 0.7F, GuiColor.RED.getHexColor());
				}
			}
			gui.drawString(gui.getLinesFromText(Translator.translate("hqm.party.stats"), 0.7F, 70), 177, 192, 0.7F,
					0x404040);
			...
			gui.drawRect(INFO_BOX_X, infoY + REWARD_SETTING_Y, INFO_BOX_SRC_X, INFO_BOX_SRC_Y, INFO_BOX_SIZE,
					INFO_BOX_SIZE);

			gui.drawString(Translator.translate("hqm.party.lifeSetting", infoTeam.getLifeSetting().getTitle()),
					INFO_BOX_X + INFO_BOX_TEXT_OFFSET_X, infoY + INFO_BOX_TEXT_OFFSET_Y, 0.7F, 0x404040);
			gui.drawString(Translator.translate("hqm.party.rewardSetting", infoTeam.getRewardSetting().getTitle()),
					INFO_BOX_X + INFO_BOX_TEXT_OFFSET_X, infoY + REWARD_SETTING_Y + INFO_BOX_TEXT_OFFSET_Y, 0.7F,
					0x404040);
	...
	public void drawMouseOver(GuiBase gui, int mX, int mY) {
		...
		if (!team.isSingle() || inviteTeam != null) {
			...
			if (gui.inBounds(INFO_BOX_X, infoY, INFO_BOX_SIZE, INFO_BOX_SIZE, mX, mY)) {
				gui.drawMouseOver(gui.getLinesFromText(
						GuiColor.GREEN + infoTeam.getLifeSetting().getTitle() + "\n"
								+ infoTeam.getLifeSetting().getDescription()
								+ (isOwner ? "\n\n" + GuiColor.ORANGE + Translator.translate("hqm.party.change") : ""),
						1F, 200), gui.getLeft() + mX, gui.getTop() + mY);
			}
			else if (gui.inBounds(INFO_BOX_X, infoY + REWARD_SETTING_Y, INFO_BOX_SIZE, INFO_BOX_SIZE, mX, mY)) {
				gui.drawMouseOver(gui.getLinesFromText(
						GuiColor.GREEN + infoTeam.getRewardSetting().getTitle() + "\n"
								+ infoTeam.getRewardSetting().getDescription()
								+ (isOwner ? "\n\n" + GuiColor.ORANGE + Translator.translate("hqm.party.change") : ""),
						1F, 200), gui.getLeft() + mX, gui.getTop() + mY);
			}
		}
		...
		if (Team.latestError != null) {
			if (inviteButton.inButtonBounds(gui, mX, mY)) {
				gui.drawMouseOver(gui.getLinesFromText(
						GuiColor.RED + Team.latestError.getHeader() + "\n" + Team.latestError.getMessage(), 1F, 150),
						mX + gui.getLeft(), mY + gui.getTop());
			}
			else {
	...
	public void onClick(GuiBase gui, int mX, int mY, int b) {
		...
		if (team.isSingle() && inviteTeam == null) {
			...
			if (invites != null) {
				...
				for (int i = start; i < end; i++) {
					Team invite = invites.get(i);
					if (gui.inBounds(PLAYER_X, PLAYER_Y + PLAYER_SPACING * i,
							(int) (gui.getStringWidth(invite.getName()) * 0.7F), (int) (TEXT_HEIGHT * 0.7F), mX, mY)) {
						inviteTeam = invite;
						break;
					}
				}
			...
			int start = memberScroll.isVisible(gui)
					? Math.round((team.getPlayers().size() - VISIBLE_MEMBERS) * memberScroll.getScroll())
					: 0;
			int end = Math.min(team.getPlayers().size(), start + VISIBLE_MEMBERS);
			for (int i = start; i < end; i++) {
				Team.PlayerEntry entry = team.getPlayers().get(i);
				if (gui.inBounds(PLAYER_X, PLAYER_Y + PLAYER_SPACING * (i - start),
						(int) (gui.getStringWidth(entry.getName()) * 0.7F), (int) (TEXT_HEIGHT * 0.7F), mX, mY)) {
	...
	private Team getTeam() { return QuestingData.getQuestingData(player).getTeam(); }

	private Team.PlayerEntry getEntry(Team team) { return team.getEntry(QuestingData.getUserName(player)); }
