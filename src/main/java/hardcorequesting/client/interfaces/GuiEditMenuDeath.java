...
public class GuiEditMenuDeath extends GuiEditMenu {
	...
	public GuiEditMenuDeath(GuiQuestBook guiQuestBook, EntityPlayer player) {
		...
		scrollBar = new ScrollBar(160, 18, 186, 171, 69, PLAYERS_X) {
			@Override
			public boolean isVisible(GuiBase gui) { return DeathStats.getDeathStats().length > VISIBLE_PLAYERS; }
		};
	}
	...
	public void draw(GuiBase gui, int mX, int mY) {
		...
		for (int i = start; i < end; i++) {
			...
			gui.drawString((i + 1) + ". " + stats.getName(), PLAYERS_X, PLAYERS_Y + (i - start) * PLAYERS_SPACING,
					getColor(selected, inBounds));
			String deaths = String.valueOf(stats.getTotalDeaths());
			gui.drawString(deaths, DEATHS_RIGHT - gui.getStringWidth(deaths), PLAYERS_Y + (i - start) * PLAYERS_SPACING,
					0x404040);
		}

		gui.drawString(Translator.translate(BEST_LABEL), BEST_X, LABEL_Y,
				getColor(showBest, gui.inBounds(BEST_X, LABEL_Y, gui.getStringWidth(BEST_LABEL), 9, mX, mY)));
		gui.drawString(Translator.translate(TOTAL_LABEL), TOTAL_X, LABEL_Y,
		...
		if (stats != null) {
			...
			for (int i = 0; i < DeathType.values().length; i++) {
				...
				gui.drawRect(TYPE_LOCATION_X + TYPE_SPACING_X * x, TYPE_LOCATION_Y + TYPE_SPACING_Y * y,
						BACKGROUND_SRC_X, BACKGROUND_SRC_Y, BACKGROUND_SIZE, BACKGROUND_SIZE);
				gui.drawRect(TYPE_LOCATION_X + TYPE_SPACING_X * x + ICON_OFFSET,
						TYPE_LOCATION_Y + TYPE_SPACING_Y * y + ICON_OFFSET, ICON_SRC_X + ICON_SIZE * x,
						ICON_SRC_Y + ICON_SIZE * y, ICON_SIZE, ICON_SIZE);
			}

			gui.drawString(stats.getName(), PLAYER_INFO_X, PLAYER_INFO_Y, 0x404040);
			gui.drawString(Translator.translate("hqm.deathMenu.total", stats.getTotalDeaths()), PLAYER_INFO_X,
					PLAYER_INFO_Y + PLAYER_TOTAL_DEATHS_Y, 0.7F, 0x404040);
			...
			for (int i = 0; i < DeathType.values().length; i++) {
				...
				gui.drawString(str, TYPE_LOCATION_X + TYPE_SPACING_X * x + TEXT_OFFSET_X,
						TYPE_LOCATION_Y + TYPE_SPACING_Y * y + TEXT_OFFSET_Y + offset, f, 0x404040);
			}
	...
	public void drawMouseOver(GuiBase gui, int mX, int mY) {
		...
		if (stats != null) {
			for (int i = 0; i < DeathType.values().length; i++) {
				...
				if (gui.inBounds(TYPE_LOCATION_X + TYPE_SPACING_X * x, TYPE_LOCATION_Y + TYPE_SPACING_Y * y,
						BACKGROUND_SIZE, BACKGROUND_SIZE, mX, mY)) {
	...
	@Override
	public void onDrag(GuiBase gui, int mX, int mY) { scrollBar.onDrag(gui, mX, mY); }

	@Override
	public void onRelease(GuiBase gui, int mX, int mY) { scrollBar.onRelease(gui, mX, mY); }

	@Override
	public void onScroll(GuiBase gui, int mX, int mY, int scroll) { scrollBar.onScroll(gui, mX, mY, scroll); }
