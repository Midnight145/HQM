...
public abstract class GuiEditMenu {
	...
	protected GuiEditMenu(final GuiBase gui, EntityPlayer player, boolean isControlOnFirstPage) {
		...
		buttons.add(new LargeButton("hqm.edit.cancel", xOffset + 100, 200) {
			...
			@Override
			public void onClick(GuiBase gui, EntityPlayer player) { close(gui); }
		});
	}
	...
	protected void close(GuiBase gui) { gui.setEditMenu(null); }
	...
	public boolean doesRequiredDoublePage() { return true; }

	public boolean hasButtons() { return hasButtons; }
	...
	public abstract class CheckBox {
		...
		protected void draw(GuiBase gui, int mX, int mY) {
			...
			gui.drawRect(x, y, CHECK_BOX_SRC_X + (selected ? CHECK_BOX_SIZE : 0),
					CHECK_BOX_SRC_Y + (hover ? CHECK_BOX_SIZE : 0), CHECK_BOX_SIZE, CHECK_BOX_SIZE);
