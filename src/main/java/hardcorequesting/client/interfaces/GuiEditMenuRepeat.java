...
public class GuiEditMenuRepeat extends GuiEditMenuExtended {
	...
	public GuiEditMenuRepeat(GuiQuestBook gui, EntityPlayer player, Quest quest) {
		...
		textBoxes.add(new TextBoxHidden(gui, 0, "hqm.repeatMenu.days") {
			@Override
			protected int getValue() { return days; }

			@Override
			protected void setValue(int number) { days = number; }
		});

		textBoxes.add(new TextBoxHidden(gui, 1, "hqm.repeatMenu.hours") {
			@Override
			protected int getValue() { return hours; }

			@Override
			protected void setValue(int number) { hours = number; }

			@Override
			protected void draw(GuiBase gui, boolean selected) {
				super.draw(gui, selected);

				gui.drawString(gui.getLinesFromText(Translator.translate("hqm.repeatMenu.mcDaysHours"), 0.7F, 150),
						BOX_X, BOX_Y + BOX_OFFSET * 2 + TEXT_OFFSET, 0.7F, 0x404040);
			}
		});
	}

	private abstract class TextBoxHidden extends TextBoxNumber {
		public TextBoxHidden(GuiQuestBook gui, int id, String title) { super(gui, id, title); }

		@Override
		protected boolean isVisible() { return type.isUseTime(); }
	}
	...
	@Override
	protected String getArrowText() { return type.getName(); }

	@Override
	protected String getArrowDescription() { return type.getDescription(); }
}
