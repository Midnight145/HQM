...
public class GuiEditMenuReputationSetting extends GuiEditMenuExtended {
	...
	public GuiEditMenuReputationSetting(GuiQuestBook gui, EntityPlayer player, QuestTaskReputation task, int id,
		...
		checkboxes.add(new CheckBox("hqm.repSetting.invRange", 21, 124) {
			@Override
			public boolean getValue() { return inverted; }

			@Override
			public void setValue(boolean val) { inverted = val; }

			@Override
			protected boolean isVisible() { return reputation != null; }
		});
	}
	...
	public void draw(GuiBase gui, int mX, int mY) {
		...
		if (reputation != null) {
			...
			info = reputation.draw((GuiQuestBook) gui, BARS_X, LOWER_Y + BAR_OFFSET_Y, mX, mY, info, player, false,
					null, null, false, lower, lower == null ? "" : "Selected: " + lower.getLabel(), false);

			gui.drawString(Translator.translate("hqm.repSetting.upper"), BARS_X, UPPER_Y, 0x404040);
			gui.applyColor(0xFFFFFFFF);
			ResourceHelper.bindResource(GuiBase.MAP_TEXTURE);
			info = reputation.draw((GuiQuestBook) gui, BARS_X, UPPER_Y + BAR_OFFSET_Y, mX, mY, info, player, false,
					null, null, false, upper, upper == null ? "" : "Selected: " + upper.getLabel(), false);

			gui.drawString(Translator.translate("hqm.repSetting.preview"), BARS_X, RESULT_Y, 0x404040);
			gui.applyColor(0xFFFFFFFF);
			ResourceHelper.bindResource(GuiBase.MAP_TEXTURE);
			info = reputation.draw((GuiQuestBook) gui, BARS_X, RESULT_Y + BAR_OFFSET_Y, mX, mY, info, player, true,
					lower, upper, inverted, null, null, false);
	...
	public void onClick(GuiBase gui, int mX, int mY, int b) {
		...
		if (reputation != null) {
			ReputationMarker marker = reputation.onActiveClick((GuiQuestBook) gui, BARS_X, LOWER_Y + BAR_OFFSET_Y, mX,
					mY);
			if (marker != null) {
