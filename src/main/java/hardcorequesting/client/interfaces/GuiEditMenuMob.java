...
public class GuiEditMenuMob extends GuiEditMenuExtended {
	...
	public GuiEditMenuMob(GuiQuestBook gui, QuestTaskMob task, final QuestTaskMob.Mob mob, int id,
		...
		scrollBar = new ScrollBar(160, 18, 186, 171, 69, START_X) {
			@Override
			public boolean isVisible(GuiBase gui) { return mobs.size() > VISIBLE_MOBS; }
		};

		textBoxes.add(new TextBoxNumber(gui, 0, "hqm.mobTask.reqKills") {
			@Override
			protected void setValue(int number) { mob.setCount(number); }

			@Override
			protected int getValue() { return mob.getCount(); }
		});
	...
	public void draw(GuiBase gui, int mX, int mY) {
		...
		for (int i = start; i < end; i++) {
			...
			gui.drawString(mobs.get(i), START_X, START_Y + OFFSET_Y * (i - start), 0.7F,
	...
	@Override
	protected void onArrowClick(boolean left) { mob.setExact(!mob.isExact()); }
