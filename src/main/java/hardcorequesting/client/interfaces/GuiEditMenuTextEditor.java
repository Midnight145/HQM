...
public class GuiEditMenuTextEditor extends GuiEditMenu {
	...
	protected GuiEditMenuTextEditor(GuiQuestBook gui, EntityPlayer player, String txt, boolean isName) {
		...
		this.text.setMaxLength(isName ? DataBitHelper.QUEST_NAME_LENGTH.getMaximum()
		...
		buttons.add(new LargeButton("hqm.textEditor.copyAll", 185, 20) {
			...
			@Override
			public void onClick(GuiBase gui, EntityPlayer player) { GuiScreen.setClipboardString(text.getText()); }
		});
		...
		buttons.add(new LargeButton("hqm.textEditor.paste", 245, 20) {
			...
			@Override
			public void onClick(GuiBase gui, EntityPlayer player) { text.addText(gui, GuiScreen.getClipboardString()); }
		});
		...
		buttons.add(new LargeButton("hqm.textEditor.clear", 185, 40) {
			...
			@Override
			public void onClick(GuiBase gui, EntityPlayer player) { text.setTextAndCursor(gui, ""); }
		});
	...
	public void draw(GuiBase gui, int mX, int mY) {
		...
		gui.drawCursor(START_X + text.getCursorPositionX(gui) - 1,
				START_Y + text.getCursorPositionY(gui) - 3 - page * LINES_PER_PAGE * TEXT_HEIGHT, 10, 1F, 0xFF909090);
	}
