...
public class GuiEditMenuParentCount extends GuiEditMenuExtended {
	...
	public GuiEditMenuParentCount(GuiBase gui, EntityPlayer player, Quest quest) {
		...
		textBoxes.add(new TextBoxNumber(gui, 0, "hqm.parentCount.count") {
			@Override
			protected void setValue(int number) { parentRequirementCount = number; }

			@Override
			protected int getValue() { return parentRequirementCount; }

			@Override
			protected boolean isVisible() { return useModifiedParentRequirement; }
		});
	}

	@Override
	protected void onArrowClick(boolean left) { useModifiedParentRequirement = !useModifiedParentRequirement; }
