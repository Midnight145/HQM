...
public abstract class LargeButton {
	...
	public void draw(GuiBase gui, EntityPlayer player, int mX, int mY) {
		if (isVisible(gui, player)) {
			...
			gui.drawRect(x, y, BUTTON_SRC_X + (enabled && inButtonBounds(gui, mX, mY) ? BUTTON_WIDTH : 0), BUTTON_SRC_Y,
	...
	protected String getName() { return Translator.translate(name); }

	protected String getDescription() { return Translator.translate(description); }
