...
public class GuiReward extends GuiBase {
	...
	public void drawScreen(int mX0, int mY0, float f) {
		...
		drawCenteredString(Translator.translate("hqm.rewardGui.close"), 0, TOP_HEIGHT + lines * MIDDLE_HEIGHT, 0.7F,
				TEXTURE_WIDTH, BOTTOM_HEIGHT, 0x707070);

		for (Reward reward : rewards) {
			try {
				drawItem(reward.item, reward.x, reward.y, true);
				itemRenderer.renderItemOverlayIntoGUI(fontRendererObj, Minecraft.getMinecraft().getTextureManager(),
						reward.item, reward.x + left + 1, reward.y + top + 1);
			}
			...
			if (inBounds(reward.x, reward.y, ITEM_SIZE, ITEM_SIZE, mX, mY)) {
				try {
					if (GuiScreen.isShiftKeyDown()) {
						drawMouseOver(reward.item.getTooltip(Minecraft.getMinecraft().thePlayer,
								Minecraft.getMinecraft().gameSettings.advancedItemTooltips), mX0, mY0);
					}
					else {
						List<String> str = new ArrayList<>();
						try {
							List info = reward.item.getTooltip(Minecraft.getMinecraft().thePlayer,
