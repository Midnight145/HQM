...
public class GuiQuestBook extends GuiBase {
...

	{
		...
		this.scrollBars.add(this.descriptionScroll = new ScrollBar(312, 18, 64, 249, 102, DESCRIPTION_X) {
			...
			public boolean isVisible(GuiBase gui) {
				return !isReputationPage && !isBagPage && !isMainPageOpen && selectedSet != null && !isSetOpened
						&& selectedSet.getDescription(gui).size() > VISIBLE_DESCRIPTION_LINES;
			}
		});
		...
		this.scrollBars.add(this.setScroll = new ScrollBar(160, 18, 186, 171, 69, LIST_X) {
			...
			public boolean isVisible(GuiBase gui) {
				return !isReputationPage && !isBagPage && !isMainPageOpen && (selectedSet == null || !isSetOpened)
						&& Quest.getQuestSets().size() > VISIBLE_SETS;
			}
		});
		...
		this.scrollBars.add(this.mainDescriptionScroll = new ScrollBar(312, 18, 186, 171, 69, DESCRIPTION_X) {
			@Override
			public boolean isVisible(GuiBase gui) {
				return !isReputationPage && !isBagPage && isMainPageOpen
						&& Quest.getMainDescription(gui).size() > VISIBLE_MAIN_DESCRIPTION_LINES;
			}
		});
		...
		this.scrollBars.add(this.groupScroll = new ScrollBar(160, 18, 186, 171, 69, GROUPS_X) {
			@Override
			public boolean isVisible(GuiBase gui) {
				return !isReputationPage && isBagPage && selectedGroup == null
						&& Group.getGroups().size() > VISIBLE_GROUPS;
			}
		});
		...
		this.scrollBars.add(this.tierScroll = new ScrollBar(312, 18, 186, 171, 69, TIERS_X) {
			...
			public boolean isVisible(GuiBase gui) {
				return !isReputationPage && isBagPage && selectedGroup == null
						&& GroupTier.getTiers().size() > VISIBLE_TIERS;
			}
		});

		this.scrollBars.add(
				this.reputationTierScroll = new ScrollBar(312, 23, 186, 171, 69, Reputation.REPUTATION_MARKER_LIST_X) {
					@Override
					public boolean isVisible(GuiBase gui) {
						return isReputationPage && !isBagPage && !isMainPageOpen && selectedReputation != null
		...
		this.scrollBars.add(this.reputationScroll = new ScrollBar(160, 23, 186, 171, 69, Reputation.REPUTATION_LIST_X) {
			...
			public boolean isVisible(GuiBase gui) {
				return isReputationPage && !isBagPage
						...
						&& Reputation.getReputationList().size() > VISIBLE_REPUTATIONS;
			}
		});
		...
		this.scrollBars.add(this.reputationDisplayScroll = new ScrollBar(160, 125, 87, 164, 69, INFO_LEFT_X) {
			...
			public boolean isVisible(GuiBase gui) {
				return isMenuPageOpen && !isMainPageOpen
						&& Reputation.getReputationList().size() > VISIBLE_DISPLAY_REPUTATIONS;
			}
		});
	}
	...
	public TextBoxGroup.TextBox getTextBoxGroupAmount() { return this.textBoxGroupAmount; }
...

	{
		...
		this.textBoxes.add(this.textBoxGroupAmount = new TextBoxGroup.TextBox(this, "0", 180, 30, false) {
			@Override
			protected boolean isCharacterValid(char c) { return this.getText().length() < 3 && Character.isDigit(c); }
	...
	public static Group getSelectedGroup() { return selectedGroup; }

	@Override
	public void setEditMenu(GuiEditMenu editMenu) { this.editMenu = editMenu; }
...

	{
		this.buttons.add(this.saveButton = new LargeButton("hqm.questBook.saveAll", 360, 10) {
			...
			@Override
			public void onClick(GuiBase gui, EntityPlayer player) { GuiQuestBook.this.save(); }
		});
		...
		this.buttons.add(new LargeButton("hqm.questBook.open", 245, 190) {
			...
			@Override
			public boolean isVisible(GuiBase gui, EntityPlayer player) {
				return GuiQuestBook.this.editMenu == null && selectedSet != null && !isBagPage && !isSetOpened
						&& !isMainPageOpen && !isMenuPageOpen && !isReputationPage;
			}

			@Override
			public void onClick(GuiBase gui, EntityPlayer player) { isSetOpened = true; }
		});
		...
		this.buttons.add(new LargeButton("hqm.questBook.createSet", 185, 50) {
			...
			public boolean isVisible(GuiBase gui, EntityPlayer player) {
				return GuiQuestBook.this.editMenu == null && Quest.isEditing
						&& GuiQuestBook.this.currentMode == EditMode.CREATE && !isBagPage && !isSetOpened
						&& !isMainPageOpen && !isMenuPageOpen && !isReputationPage;
			}
			...
			public void onClick(GuiBase gui, EntityPlayer player) {
				...
				for (final QuestSet set : Quest.getQuestSets()) {
					if (set.getName().startsWith("Unnamed set")) {
						i++;
					}
				}
		...
		this.buttons.add(new LargeButton("hqm.questBook.createGroup", 100, 175) {
			@Override
			public boolean isEnabled(GuiBase gui, EntityPlayer player) {
				return GroupTier.getTiers().size() > 0
						&& Group.getGroups().size() < DataBitHelper.GROUP_COUNT.getMaximum();
			}

			@Override
			public boolean isVisible(GuiBase gui, EntityPlayer player) {
				return GuiQuestBook.this.editMenu == null && isBagPage
						&& GuiQuestBook.this.currentMode == EditMode.CREATE && selectedGroup == null && !isMainPageOpen
						&& !isMenuPageOpen && !isReputationPage;
			}
		...
		this.buttons.add(new LargeButton("Reset", 90, 190) {
			...
			@Override
			public boolean isVisible(GuiBase gui, EntityPlayer player) {
				return GuiQuestBook.this.editMenu == null && !isBagPage && !isMainPageOpen && GuiQuestBook.this.isOpBook
						&& isMenuPageOpen && !isReputationPage;
			}

			@Override
			public void onClick(GuiBase gui, EntityPlayer player) { OPBookHelper.reset(player); }
		});
		...
		this.buttons.add(new LargeButton("Create New", 180, 20) {
			...
			public boolean isVisible(GuiBase gui, EntityPlayer player) {
				return GuiQuestBook.this.editMenu == null && !isBagPage
						&& GuiQuestBook.this.currentMode == EditMode.CREATE && selectedReputation == null
						&& !isMainPageOpen && !isMenuPageOpen && isReputationPage;
			}
	...
	public void drawScreen(int x0, int y0, float f) {
		...
		if (this.shouldDisplayControlArrow(false)) {
			this.drawRect(BACK_ARROW_X, BACK_ARROW_Y,
					BACK_ARROW_SRC_X + (this.inArrowBounds(false, x, y) ? BACK_ARROW_WIDTH : 0), BACK_ARROW_SRC_Y,
					BACK_ARROW_WIDTH, BACK_ARROW_HEIGHT);
		}
		if (this.shouldDisplayControlArrow(true)) {
			this.drawRect(MENU_ARROW_X, MENU_ARROW_Y,
					MENU_ARROW_SRC_X + (this.inArrowBounds(true, x, y) ? MENU_ARROW_WIDTH : 0), MENU_ARROW_SRC_Y,
		...
		if (this.editMenu == null) {
			...
			if (this.currentMode == EditMode.DELETE) {
				...
				this.drawCenteredString(Translator.translate("hqm.questBook.warning"), 0, 0, 2F, TEXTURE_WIDTH,
						TEXTURE_HEIGHT, 0xFF0000);
				this.drawCenteredString(Translator.translate("hqm.questBook.deleteOnClick"), 0,
						this.fontRendererObj.FONT_HEIGHT * 2, 1F, TEXTURE_WIDTH, TEXTURE_HEIGHT, 0xFF0000);
				this.applyColor(0xFFFFFFFF);
				ResourceHelper.bindResource(MAP_TEXTURE);
				GL11.glPopMatrix();
			}
		...
		if (this.shouldDisplayAndIsInArrowBounds(false, x, y)) {
			this.drawMouseOver(Translator.translate("hqm.questBook.goBack") + "\n" + GuiColor.GRAY
					+ Translator.translate("hqm.questBook.rightClick"), x + this.left, y + this.top);
		}
	...
	private void drawMenuPage(int x, int y) {
		...
		this.drawString(Translator.translate("hqm.questBook.showQuests"), INFO_LEFT_X,
				INFO_QUESTS_Y + QUEST_CLICK_TEXT_Y, 0.7F, 0x707070);

		if (QuestingData.isHardcoreActive()) {
			final boolean almostOut = QuestingData.getQuestingData(this.player).getLives() == QuestingData
					.getQuestingData(this.player).getLivesToStayAlive();
			if (almostOut) {
				this.drawString(GuiColor.RED + Translator.translate("hqm.questBook.deadOut"), INFO_RIGHT_X + 50,
						INFO_LIVES_Y + 2, 0.7F, 0x404040);
			}

			GL11.glColor4f(1, 1, 1, 1);
			final int lives = QuestingData.getQuestingData(this.player).getLives();
			...
			for (int i = 0; i < count; i++) {
				this.drawItem(new ItemStack(ModItems.hearts, 1, 3), heartX + spacing * i, INFO_LIVES_Y + INFO_HEARTS_Y,
						almostOut);
			}
		}
		else {
			this.drawString(
					...
					INFO_RIGHT_X, INFO_LIVES_Y + 12, 0.5F, 0x707070);
		}

		final int deaths = DeathStats.getDeathStats(QuestingData.getUserName(this.player)).getTotalDeaths();
		this.drawString(Translator.translate(deaths != 1, "hqm.questBook.deaths", deaths), INFO_RIGHT_X,
				INFO_DEATHS_Y + DEATH_TEXT_Y, 0.7F, 0x404040);
		this.drawString(Translator.translate("hqm.questBook.moreInfo"), INFO_RIGHT_X,
				INFO_DEATHS_Y + DEATH_CLICK_TEXT_Y, 0.7F, 0x707070);

		String str;
		final Team team = QuestingData.getQuestingData(this.player).getTeam();
		if (team.isSingle()) {
			final int invites = team.getInvites() == null ? 0 : team.getInvites().size();
			if (invites > 0) {
		...
		this.drawString(Translator.translate("hqm.questBook.openParty"), INFO_RIGHT_X, INFO_TEAM_Y + TEAM_CLICK_TEXT_Y,
				0.7F, 0x707070);

		if (this.isOpBook) {
			this.drawString(Translator.translate("hqm.questBook.resetParty"), 22, 182, 0.6F, 0x404040);
			this.drawString(this.getLinesFromText(Translator.translate("hqm.questBook.shiftCtrlConfirm"), 0.6F, 70), 22,
					192, 0.6F, GuiColor.RED.getHexColor());
		}

		Reputation.drawAll(this, INFO_LEFT_X + INFO_REPUTATION_OFFSET_X, INFO_REPUTATION_Y + INFO_REPUTATION_OFFSET_Y,
				x, y, this.player);
	}

	private void drawMainPage() {
		final int startLine = this.mainDescriptionScroll.isVisible(this)
				? Math.round((Quest.getMainDescription(this).size() - VISIBLE_MAIN_DESCRIPTION_LINES)
						* this.mainDescriptionScroll.getScroll())
				: 0;
		this.drawString(Quest.getMainDescription(this), startLine, VISIBLE_MAIN_DESCRIPTION_LINES, DESCRIPTION_X,
				DESCRIPTION_Y, 0.7F, 0x404040);
		this.drawCenteredString(Translator.translate("hqm.questBook.start"), 0, 195, 0.7F, PAGE_WIDTH,
				TEXTURE_HEIGHT - 195, 0x707070);
		if (SoundHandler.hasLoreMusic() && !SoundHandler.isLorePlaying()) {
			this.drawCenteredString(Translator.translate("hqm.questBook.playAgain"), PAGE_WIDTH, 195, 0.7F,
					PAGE_WIDTH - 10, TEXTURE_HEIGHT - 195, 0x707070);
		}
		if (QuestLine.getActiveQuestLine().front == null && QuestLine.getActiveQuestLine().mainPath != null) {
			...
			if (file.exists()) {
				try {
					...
					QuestLine.getActiveQuestLine().front = Minecraft.getMinecraft().getTextureManager()
	...
	private boolean isNEIActive() { return isNEIActive; }

	public static void setSelected(ItemStack stack) { selected = stack; }
	...
	private void menuPageMouseClicked(int button, int x, int y) {
		if (button == 1) {
			...
			if (this.inBounds(INFO_RIGHT_X, INFO_TEAM_Y + TEAM_CLICK_TEXT_Y, PAGE_WIDTH, (int) (TEXT_HEIGHT * 0.7F), x,
					y)) {
				this.editMenu = new GuiEditMenuTeam(this, this.player);
			}
			else if (this.inBounds(INFO_RIGHT_X, INFO_DEATHS_Y + DEATH_CLICK_TEXT_Y, PAGE_WIDTH,
					(int) (TEXT_HEIGHT * 0.7F), x, y)) {
				this.editMenu = new GuiEditMenuDeath(this, this.player);
			}
			else if (this.inBounds(INFO_LEFT_X, INFO_QUESTS_Y + QUEST_CLICK_TEXT_Y, PAGE_WIDTH,
					(int) (TEXT_HEIGHT * 0.7F), x, y)) {
				isMenuPageOpen = false;
			}
		}
	...
	private void mainPageMouseClicked(int x, int y) {
		if (x > 0 && x < PAGE_WIDTH && y > 205) {
			...
			if (Quest.isEditing && this.currentMode == EditMode.RENAME && this.inBounds(DESCRIPTION_X, DESCRIPTION_Y,
					130, (int) (VISIBLE_MAIN_DESCRIPTION_LINES * TEXT_HEIGHT * 0.7F), x, y)) {
	...
	public void loadMap() { selectedQuest = null; }

	private EditMode currentMode = EditMode.NORMAL;

	public EditMode getCurrentMode() { return this.currentMode; }
	...
	private final EditButton[] groupButtons = EditButton.createButtons(this, EditMode.NORMAL, EditMode.ITEM,
			EditMode.DELETE);
	private final EditButton[] bagButtons = EditButton.createButtons(this, EditMode.NORMAL, EditMode.CREATE,
			EditMode.RENAME, EditMode.TIER, EditMode.DELETE);
	private final EditButton[] reputationButtons = EditButton.createButtons(this, EditMode.NORMAL, EditMode.CREATE,
			EditMode.RENAME, EditMode.REPUTATION_VALUE, EditMode.DELETE);
	private final EditButton[] mainButtons = EditButton.createButtons(this, EditMode.NORMAL, EditMode.RENAME);
	private final EditButton[] menuButtons = EditButton.createButtons(this, EditMode.NORMAL, EditMode.BAG,
			EditMode.REPUTATION);
	private final EditButton[] overviewButtons = EditButton.createButtons(this, EditMode.NORMAL, EditMode.CREATE,
			EditMode.RENAME, EditMode.SWAP_SELECT, EditMode.DELETE);
	private final EditButton[] setButtons = EditButton.createButtons(this, EditMode.NORMAL, EditMode.MOVE,
			EditMode.CREATE, EditMode.REQUIREMENT, EditMode.SIZE, EditMode.ITEM, EditMode.REPEATABLE, EditMode.TRIGGER,
			EditMode.REQUIRED_PARENTS, EditMode.QUEST_SELECTION, EditMode.QUEST_OPTION, EditMode.SWAP,
			EditMode.REP_BAR_CREATE, EditMode.REP_BAR_CHANGE, EditMode.DELETE);
	private final EditButton[] questButtons = EditButton.createButtons(this, EditMode.NORMAL, EditMode.RENAME,
			EditMode.TASK, EditMode.CHANGE_TASK, EditMode.ITEM, EditMode.LOCATION, EditMode.MOB,
			EditMode.REPUTATION_TASK, EditMode.REPUTATION_REWARD, EditMode.COMMAND_CREATE, EditMode.COMMAND_CHANGE,
			EditMode.DELETE);
