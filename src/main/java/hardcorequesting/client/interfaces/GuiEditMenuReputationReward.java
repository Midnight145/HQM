...
public class GuiEditMenuReputationReward extends GuiEditMenuExtended {
	...
	public GuiEditMenuReputationReward(GuiBase gui, EntityPlayer player, List<ReputationReward> rewards) {
		...
		textBoxes.add(new TextBoxNumber(gui, 0, "hqm.repReward.value") {
			@Override
			protected int getValue() { return selectedReward.getValue(); }

			@Override
			protected void setValue(int number) { selectedReward.setValue(number); }

			@Override
			protected boolean isVisible() { return selectedReward != null; }
		...
		buttons.add(new LargeButton("hqm.repReward.create", 20, 20) {
			...
			@Override
			public boolean isVisible(GuiBase gui, EntityPlayer player) { return isValid(); }

			@Override
			public void onClick(GuiBase gui, EntityPlayer player) {
				GuiEditMenuReputationReward.this.rewards
						.add(new ReputationReward(Reputation.getReputationList().get(0), 0));
			}
		});
		...
		buttons.add(new LargeButton("hqm.repReward.delete", 80, 20) {
			...
			@Override
			public boolean isVisible(GuiBase gui, EntityPlayer player) { return isValid() && selectedReward != null; }
	...
	public void draw(GuiBase gui, int mX, int mY) {
		...
		if (isValid()) {
			for (int i = 0; i < rewards.size(); i++) {
				...
				gui.drawString(str, START_X, START_Y + i * OFFSET,
	...
	public void onClick(GuiBase gui, int mX, int mY, int b) {
		...
		if (isValid()) {
			for (int i = 0; i < rewards.size(); i++) {
				if (gui.inBounds(START_X, START_Y + i * OFFSET, gui.getStringWidth(rewards.get(i).getLabel()), 9, mX,
						mY)) {
	...
	private boolean isValid() { return !Reputation.getReputationList().isEmpty(); }

	@Override
	protected boolean isArrowVisible() { return isValid() && selectedReward != null; }
	...
	@Override
	protected String getArrowText() { return selectedReward.getReward().getName(); }

	@Override
	protected String getArrowDescription() { return null; }
