...
public class GuiBase extends GuiScreen {
	...
	public void drawRect(int x, int y, int u, int v, int w, int h, RenderRotation rotation) {
		boolean rotate = rotation == RenderRotation.ROTATE_90 || rotation == RenderRotation.ROTATE_270
				|| rotation == RenderRotation.ROTATE_90_FLIP || rotation == RenderRotation.ROTATE_270_FLIP;
	...
	public void drawIcon(IIcon icon, int x, int y) { drawTexturedModelRectFromIcon(left + x, top + y, icon, 16, 16); }
	...
	public void drawItem(ItemStack item, int x, int y, int mX, int mY, boolean selected) {
		...
		if (item != null && item.getItem() != null) {
			...
			itemRenderer.renderItemOverlayIntoGUI(fontRendererObj, Minecraft.getMinecraft().getTextureManager(), item,
					x + left + 1, y + +top + 1);
		}
	...
	public void drawItem(ItemStack itemstack, int x, int y, boolean renderEffect) {
		...
		setZLevel(4f);
		try {
			if (!ForgeHooksClient.renderInventoryItem(blockRenderer, this.mc.getTextureManager(), itemstack,
					renderEffect, zLevel, x + left, y + top)) {
				itemRenderer.renderItemAndEffectIntoGUI(fontRendererObj, this.mc.getTextureManager(), itemstack,
						x + left, y + top);
			}
		}
		finally {
	...
	public int getStringWidth(String txt) { return fontRendererObj.getStringWidth(txt); }

	public void drawString(String str, int x, int y, int color) { drawString(str, x, y, 1F, color); }
	...
	public void drawCenteredString(String str, int x, int y, float mult, int width, int height, int color) {
		drawString(str, x + (width - (int) (fontRendererObj.getStringWidth(str) * mult)) / 2,
				y + (height - (int) ((fontRendererObj.FONT_HEIGHT - 2) * mult)) / 2, mult, color);
	}
