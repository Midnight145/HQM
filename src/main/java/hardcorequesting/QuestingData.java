...
public class QuestingData {
	...
	static HashMap<String, QuestingData> getData() { return data; }

	public DeathStats getDeathStat() { return deathStat; }
	...
	public int getRawLives() { return lives; }

	public QuestData getQuestData(int id) { return getTeam().getQuestData(id); }
		...
		if (getTeam().isSharingLives()) {
			for (Team.PlayerEntry entry : getTeam().getPlayers()) {
				if (entry.isInTeam() && !entry.getName().equals(getUserName(player))) {
					...
					if (other != null) {
						other.addChatMessage(new ChatComponentText(Translator.translate(getLives() != 1,
								"hqm.message.lostTeamLife", getUserName(player),
	...
	/**
	 * Deletes the world or bans the player from the server. This is handled on the
	 * server side
	 *
	...
	private void outOfLives(EntityPlayer playerEntity) {
		...
		if (mcServer.isSinglePlayer() && playerEntity.getCommandSenderName().equals(mcServer.getServerOwner())) {
			 ...
			 *
			 * mcServer.getActiveAnvilConverter().deleteWorldDirectory(mcServer.worldServers
			 * [0].getSaveHandler().getWorldDirectoryName()); mcServer.initiateShutdown();
			 */
	...
	public static boolean isHardcoreActive() { return hardcoreActive; }

	public static boolean isQuestActive() { return questActive; }

	/*
	 * public static void activateDebug() { if(!debugActive) debugActive = true; }
	 *
	 * public static void deactivateDebug() { if(debugActive) debugActive = false; }
	 */
	...
	public static void disableHardcore() { hardcoreActive = false; }
	...
	public static QuestingData getQuestingData(EntityPlayer player) { return getQuestingData(getUserName(player)); }
	...
	/*
	 * This is saved as a header of the file and read again when it's loaded. This
	 * is used when it loads again so we can load it differently depending on the
	 * version. The version should manually increase if we change the way things are
	 * saved. This includes if we add extra things that has to be saved. If we can't
	 * treat different versions differently then files from old version (i.e. from
	 * before the user updated the mod) will count as corrupted since they can't be
	 * read. The version is sent as a parameter to the load constructor of the
	 * class. At the moment it's not used since there's only one version.
	 *
	 * There's NO reason to increase this number each time you develop something
	 * new. Corrupt files doesn't matter when developing since that will only be our
	 * testing files. For instance, say we've released a version with only the lives
	 * (no quest book) and call that file version 0. Then when we want to save the
	 * quest book we increase FILE_VERSION to 1 and start developing. When it's
	 * released it's released as file version 1 even though the quest book's saving
	 * might have changed a few times during the development. The important things
	 * is that version 0 is still the old thing without the quest book so it won't
	 * try to load the quest book otherwise.
	 *
	 * Files from newer versions are obviously not going to be able to be read. So
	 * if the player is still running a version with the file version of 0 but
	 * copies a file from file version 1 it's not going to works. That's very
	 * obvious and totally expected.
	 *
	 * If you have any questions about the file version I'll be happy to answer them
	 * /Vswe
	 */
	...
	private static final FileHelper FILE_HELPER = new FileHelper() {
		@Override
		public void write(DataWriter dw) { saveAllData(dw); }

		@Override
		public void read(DataReader dr, FileVersion version) { readAllData(dr, version); }
	};
	...
	public static void save(File worldPath, WorldServer world) {
		if (!(isHardcoreActive() || isQuestActive()) || Quest.isEditing) { return; }
	...
	private QuestingData(FileVersion version, DataReader dr) {
		...
		// When filling in data for a player on a team, we sometimes follow a codepath
		// that causes us to attempt to
		// access its teammates. If those teammates haven't been loaded yet, we'll
		// generate dummy versions of the
		// players and put them into the questing data db (for some reason??) then
		// we won't load the player's data when it comes time to acutally load them from
		// the file because the player is
		// already in the database (why???) and so that player will have their progress
		// wiped. If that player is
		...
		// probably be avoided by dropping a nuke on the code responsible, but you gotta
		// do what ya gotta do.
		for (Object teammateObj : team.getPlayers()) {
			...
			if (QuestingData.data.containsKey(name)) {
				QuestingData teammatePlayer = QuestingData.data.get(name);
	...
	public static void remove(EntityPlayer player) { data.remove(getUserName(player)); }

	public static boolean hasData(EntityPlayer player) { return data.containsKey(player.getGameProfile().getName()); }
