...
public class Team {
	...
	public void receiveAndSyncReputation(Quest quest, List<ReputationReward> reputationList) {
		for (ReputationReward reputationReward : reputationList) {
			setReputation(reputationReward.getReward(),
	...
	public enum LifeSetting {
		...
		public String getTitle() { return Translator.translate(title); }

		public String getDescription() { return Translator.translate(description); }
	}
	...
	public enum RewardSetting {
		...
		private static RewardSetting getDefault() { return isAllModeEnabled ? ALL : ANY; }
	...
	public RewardSetting getRewardSetting() { return rewardSetting; }

	public LifeSetting getLifeSetting() { return lifeSetting; }

	private int clientTeamLives = -1;

	public int getId() { return id; }

	public boolean isSharingLives() { return lifeSetting == LifeSetting.SHARE; }

	public int getSharedLives() {
		if (clientTeamLives != -1) { return clientTeamLives; }
	...
	public List<Team> getInvites() { return invites; }
	...
	public void leave() { PacketHandler.sendToServer(getWriter(TeamAction.LEAVE)); }

	public void disband() { PacketHandler.sendToServer(getWriter(TeamAction.DISBAND)); }

	public void nextLifeSetting() { PacketHandler.sendToServer(getWriter(TeamAction.NEXT_LIFE_SETTING)); }

	public void nextRewardSetting() { PacketHandler.sendToServer(getWriter(TeamAction.NEXT_REWARD_SETTING)); }

	// slightly ugly but there's no real way of getting hold of the interface, this
	...
	public enum ErrorMessage {
		...
		public String getMessage() { return Translator.translate(message); }

		public String getHeader() { return Translator.translate(header); }
			...
			case INVITE:
				if (!team.isSingle() && team.isOwner(playerName)) {
					...
					if (!team.players.contains(entry)) {
						...
						team.refreshTeamData(UpdateType.ONLY_MEMBERS);
						QuestingData.getQuestingData(entry.getName()).getTeam()
	...
	public PlayerEntry getEntry(String playerName) {
		for (Team.PlayerEntry playerEntry : getPlayers()) {
			if (playerEntry.getName().equals(playerName)) { return playerEntry; }
		}
	...
	public static class PlayerEntry {
		...
		public boolean shouldRefreshData() { return bookOpen || PacketHandler.getOverriddenBy(name) != null; }

		public boolean isBookOpen() { return bookOpen; }

		public void setBookOpen(boolean bookOpen) { this.bookOpen = bookOpen; }
	}
	...
	private void createReputation() {
		reputation = new ArrayList<>();
		for (int i = 0; i < Reputation.size(); i++) {
			createReputation(i);
		}
	}

	public int getReputation(Reputation reputation) { return getReputation(reputation.getId()); }

	public void setReputation(Reputation reputation, int value) { setReputation(reputation.getId(), value); }
	...
	public boolean isSingle() { return id == -1; }
	...
	private void writeTeamData(DataWriter dw, boolean light) {
		if (light) {
			...
			if (isSingle()) {
				...
				List<Team> invitingTeams = new ArrayList<>();
				for (Team team : QuestingData.getTeams()) {
