...
public enum TrackerType {
	...
	PROGRESS_CLOSE("progressClose") {
		...
		public int getMeta(TileEntityTracker tracker, Quest quest, int radius) {
			...
			for (String name : FMLCommonHandler.instance().getMinecraftServerInstance().getConfigurationManager()
				...
				if (player != null) {
					double distance = player.getDistanceSq(tracker.xCoord + 0.5, tracker.yCoord + 0.5,
							tracker.zCoord + 0.5);
	...
	TrackerType(String name) { this.id = name; }

	public String getName() { return Translator.translate("hqm.tracker." + id + ".title"); }

	public String getDescription() { return Translator.translate("hqm.tracker." + id + ".desc"); }
