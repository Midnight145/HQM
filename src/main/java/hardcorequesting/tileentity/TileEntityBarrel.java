...
public class TileEntityBarrel extends TileEntity implements IInventory, IFluidHandler {
	...
	public int fill(ForgeDirection from, FluidStack resource, boolean doFill) {
		...
		if (doFill) {
			...
			if (task != null && task instanceof QuestTaskItemsConsume) {
				if (((QuestTaskItemsConsume) task).increaseFluid(resource.copy(),
						(QuestDataTaskItems) task.getData(playerName), playerName) && modifiedSyncTimer <= 0) {
	...
	public void setInventorySlotContents(int i, ItemStack itemstack) {
		QuestTask task = getCurrentTask();
		if (task != null && task instanceof QuestTaskItemsConsume) {
			if (((QuestTaskItemsConsume) task).increaseItems(new ItemStack[] { itemstack },
					(QuestDataTaskItems) task.getData(playerName), playerName) && modifiedSyncTimer <= 0) {
				modifiedSyncTimer = SYNC_TIME;
			}
		}
	...
	@Override
	public String getInventoryName() { return BlockInfo.LOCALIZATION_START + BlockInfo.ITEMBARREL_UNLOCALIZED_NAME; }
