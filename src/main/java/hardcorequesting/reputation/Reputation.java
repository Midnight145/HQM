...
public class Reputation {
	...
	private static Map<Integer, Reputation> reputationMap = new HashMap<>();

	public static List<Reputation> getReputationList() { return reputationList; }

	public static Reputation getReputation(int id) { return reputationMap.get(id); }
	...
	public Reputation(String name, String neutralName) { this(count, name, neutralName); }
	...
	public String getNeutralName() { return neutral.getName(); }
	...
	public int getValue(EntityPlayer player) { return getValue(QuestingData.getUserName(player)); }
	...
	public static void drawAll(GuiQuestBook gui, int x, int y, int mX, int mY, final EntityPlayer player) {
		...
		List<Reputation> reputations = new ArrayList<>(reputationList);
		...
		int start = gui.reputationDisplayScroll.isVisible(gui)
				? Math.round((reputations.size() - GuiQuestBook.VISIBLE_DISPLAY_REPUTATIONS)
						* gui.reputationDisplayScroll.getScroll())
				: 0;
		...
		for (int i = start; i < end; i++) {
			...
			info = reputations.get(i).draw(gui, x, y + (i - start) * OFFSET_Y, mX, mY, info, player, false, null, null,
	...
	public String draw(GuiQuestBook gui, int x, int y, int mX, int mY, String info, EntityPlayer player,
		...
		for (int i = 0; i < markers.size(); i++) {
			...
			boolean selected = markers.get(i).equals(active)
					|| (effects && ((lowerValue <= value && value <= upperValue) != inverted));
		...
		if (drawPointer(gui, 0, x, y, ARROW_MARKER_Y, ARROW_SRC_NEUTRAL_X, mX, mY,
		...
		if (text != null) {
			...
			if (lower == null && upper == null) {
				...
				if (lower != null && upper != null) {
					if (lower.equals(upper)) {
						...
						if (inverted) {
							str = Translator.translate("hqm.rep.not") + " (" + lowerName + " <= " + name + " <= "
									+ upperName + ")";
						}
						else {
			...
			selected = completed || (effects
					&& ((lowerValue <= current.getValue() && current.getValue() <= upperValue) != inverted));
		}
	...
	private boolean drawPointer(GuiQuestBook gui, int value, int x, int y, int offsetY, int srcX, int mX, int mY,
		...
		gui.drawRect(pointerX, pointerY, srcX, ARROW_SRC_Y + (selectedTexture ? -ARROW_SIZE : 0), ARROW_SIZE,
				ARROW_SIZE);
		...
		for (int i = 0; i < markers.size(); i++) {
			...
			if (value <= marker.getValue()) {
				...
				if (pointAtMarker) {
					...
					pointerPosition = ARROW_MARKER_OFFSET + (int) ((i - 1 + ((float) internalValue / span))
							* (BAR_WIDTH - ARROW_MARKER_OFFSET * 2) / (markers.size() - 1));
				}
	...
	public static void drawEditPage(GuiQuestBook gui, int mX, int mY) {
		if (gui.getCurrentMode() != EditMode.CREATE || selectedReputation == null) {
			int start = gui.reputationScroll.isVisible(gui) ? Math.round(
					(reputationList.size() - GuiQuestBook.VISIBLE_REPUTATIONS) * gui.reputationScroll.getScroll()) : 0;
		...
		if (selectedReputation != null) {
			...
			gui.drawString(neutralName, REPUTATION_MARKER_LIST_X, REPUTATION_NEUTRAL_Y,
					gui.inBounds(REPUTATION_MARKER_LIST_X, REPUTATION_NEUTRAL_Y, gui.getStringWidth(neutralName),
							FONT_HEIGHT, mX, mY) ? 0xAAAAAA : 0x404040);

			int start = gui.reputationTierScroll.isVisible(gui)
					? Math.round((selectedReputation.markers.size() - GuiQuestBook.VISIBLE_REPUTATION_TIERS)
	...
	public ReputationMarker onActiveClick(GuiQuestBook gui, int x, int y, int mX, int mY) {
		...
		if (gui.inBounds(pointerX, pointerY, ARROW_SIZE, ARROW_SIZE, mX, mY)) {
			...
			for (int i = 0; i < markers.size(); i++) {
				...
				int markerX = x + BAR_X - ARROW_SIZE / 2 + position + ARROW_MARKER_OFFSET;
				int markerY = y + BAR_Y + ARROW_MARKER_Y;
				if (gui.inBounds(markerX, markerY, ARROW_SIZE, ARROW_SIZE, mX, mY)) { return markers.get(i); }
			}
			return null;
		}
	}
	...
	public static void onClick(GuiQuestBook gui, int mX, int mY, EntityPlayer player) {
		...
		if (selectedReputation != null) {
			String neutralName = Translator.translate("hqm.rep.neutral", selectedReputation.neutral.getName());
			if (gui.inBounds(REPUTATION_MARKER_LIST_X, REPUTATION_NEUTRAL_Y, gui.getStringWidth(neutralName),
					FONT_HEIGHT, mX, mY)) {
	...
	public boolean isValid() { return markers.size() >= 2; }
		...
		if (version.contains(FileVersion.REPUTATION)) {
			...
			for (int i = 0; i < reputationCount; i++) {
				...
				for (int j = 0; j < markerCount; j++) {
					ReputationMarker marker = new ReputationMarker(dr.readString(DataBitHelper.QUEST_NAME_LENGTH),
							dr.readData(DataBitHelper.REPUTATION_VALUE), false);
