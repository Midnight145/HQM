...
public class ReputationBar {
	...
	public QuestSet getQuestSet() { return Quest.getQuestSets().get(this.questSet); }

	public void setQuestSet(int id) { this.questSet = id; }

	public boolean isValid() {
		return Quest.getQuestSets().size() > this.questSet && getQuestSet() != null
	...
	public void draw(GuiQuestBook gui, int mX, int mY, EntityPlayer player) {
		...
		String info = reputation.draw(gui, this.x, this.y, mX, mY, null, player, false, null, null, false, null, null,
				false);
	...
	public boolean inBounds(int mX, int mY) {
		return this.x <= mX && this.x + Reputation.BAR_WIDTH >= mX && this.y - Reputation.BAR_HEIGHT * 3 <= mY
	...
	public static class EditGui extends GuiEditMenu {
		...
		public void draw(GuiBase guiB, int mX, int mY) {
			...
			int start = gui.reputationScroll.isVisible(gui)
					? Math.round((Reputation.getReputationList().size() - GuiQuestBook.VISIBLE_REPUTATIONS)
							* gui.reputationScroll.getScroll())
					: 0;
			...
			gui.drawString(gui.getLinesFromText(Translator.translate("hqm.rep.select"), 1F, 120),
					Reputation.REPUTATION_MARKER_LIST_X, Reputation.REPUTATION_LIST_Y, 1F, 0x404040);
		}
