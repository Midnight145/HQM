...
public class PacketHandler {
	...
	public static DataWriter getWriter(PacketId id) {
		...
		dw.writeByte(1); // amount of packets, defaults to 1. Hast obe a byte so it's easy to modify
						 // afterwards
	...
	public static boolean canOverride(String name) { return !nameOverrideReversed.containsKey(name); }

	public static String getOverriddenBy(String name) { return nameOverrideReversed.get(name); }
	...
	public static void closeInterface() { sendToServer(getWriter(PacketId.CLOSE_INTERFACE)); }
	...
	private void onPacketData(byte[] incomingData, EntityPlayer player, boolean onServer) {
		...
		try {
			...
			// read the packet count that was manually read before the creation of the data
			// reader
			dr.readByte();
