...
public class DataWriter {
	...
	DataWriter() { stream = new ByteArrayOutputStream(); }

	DataWriter(OutputStream stream) { this.stream = stream; }

	public void writeByte(int data) { writeData(data, DataBitHelper.BYTE); }

	public void writeBoolean(boolean data) { writeData(data ? 1 : 0, DataBitHelper.BOOLEAN); }

	public void writeData(int data, DataBitHelper bitCount) { writeData(data, bitCount.getBitCount()); }
	...
	public void sendToServer() { packetHandler.sendToServer(createPacket()); }

	public void sendToAllPlayers() { packetHandler.sendToAll(createPacket()); }

	public void sendToAllPlayersAround(TileEntity te, double range) {
		packetHandler.sendToAllAround(createPacket(), new NetworkRegistry.TargetPoint(
				te.getWorldObj().provider.dimensionId, te.xCoord + 0.5, te.yCoord + 0.5, te.zCoord, range));
	}
	...
	public void writeItem(Item item) { writeString(Item.itemRegistry.getNameForObject(item), DataBitHelper.SHORT); }
