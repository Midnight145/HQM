...
public class BlockPortal extends BlockContainer {
	...
	@Override
	public TileEntity createNewTileEntity(World world, int i) { return new TileEntityPortal(); }
	...
	@Override
	@SideOnly(Side.CLIENT)
	public void registerBlockIcons(IIconRegister icon) { pickIcons(icon); }
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7,
		...
		if (player != null && Quest.isEditing) {
			if (player.inventory.getCurrentItem() != null
					&& player.inventory.getCurrentItem().getItem() == ModItems.book) {
				if (!world.isRemote) {
					...
					if (te != null && te instanceof TileEntityPortal) {
						...
						if (((TileEntityPortal) te).getCurrentQuest() != null)
							player.addChatComponentMessage(
									Translator.translateToIChatComponent("tile.hqm:quest_portal_0.bindTo",
											((TileEntityPortal) te).getCurrentQuest().getName()));
						else
							player.addChatComponentMessage(
	...
	public void addCollisionBoxesToList(World world, int x, int y, int z, AxisAlignedBB box, List lst, Entity entity) {
		...
		if (entity instanceof EntityPlayer && te instanceof TileEntityPortal
				&& !((TileEntityPortal) te).hasCollision((EntityPlayer) entity))
			return;
	...
	@Override
	public boolean isBlockNormalCube() { return false; }
