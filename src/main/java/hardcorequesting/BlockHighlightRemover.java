...
public class BlockHighlightRemover {
	public BlockHighlightRemover() { MinecraftForge.EVENT_BUS.register(this); }

	@SubscribeEvent
	public void onRender(DrawBlockHighlightEvent event) {
		if (event.target != null && event.player != null) {
			if (ModBlocks.itemPortal == event.player.worldObj.getBlock(event.target.blockX, event.target.blockY,
					event.target.blockZ)) {
				TileEntity te = event.player.worldObj.getTileEntity(event.target.blockX, event.target.blockY,
						event.target.blockZ);
