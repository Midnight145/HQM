...
public class Group {
	...
	public static int size() { return QuestLine.getActiveQuestLine().groupCount; }
	...
	public GroupTier getTier() { return tier; }

	public static List<Group> getGroups() { return QuestLine.getActiveQuestLine().groupList; }

	public String getName() { return hasName() ? name : Translator.translate("hqm.bag.group", tier.getName()); }

	public boolean hasName() { return name != null && !name.isEmpty(); }

	public List<ItemStack> getItems() { return items; }

	public void setTier(GroupTier tier) { this.tier = tier; }
	...
	public static void saveAll(DataWriter dw) {
		...
		for (Group group : getGroups()) {
			...
			// most of the time the limit will be at 0, therefore I'm saving an extra bit in
			// the case there is a limit
	...
	public static void readAll(DataReader dr, FileVersion version) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (Quest.isEditing)
				FMLLog.log("HQM-EDIT", Level.INFO, "Loading quest group %s", name);
	...
	public static Group getGroup(int id) { return QuestLine.getActiveQuestLine().groups.get(id); }
	...
	public boolean isValid(EntityPlayer player) { return limit == 0 || getRetrievalCount(player) < limit; }
		...
		if (obj instanceof Group) {
			if (Objects.equals(name, ((Group) obj).name) && limit == ((Group) obj).limit
				...
				for (ItemStack stack : items) {
					if (!listContains(stack, ((Group) obj).items))
						return false;
				}
				return true;
			}
	...
	public static void drawOverview(GuiQuestBook gui, ScrollBar tierScroll, ScrollBar groupScroll, int x, int y) {
		...
		for (int i = start; i < Math.min(start + GuiQuestBook.VISIBLE_TIERS, tiers.size()); i++) {
			...
			boolean inBounds = gui.inBounds(GuiQuestBook.TIERS_X, yPos, gui.getStringWidth(str),
			...
			for (int j = 0; j < BagTier.values().length; j++) {
				...
				gui.drawCenteredString(bagTier.getColor().toString() + groupTier.getWeights()[j],
						...
						yPos + GuiQuestBook.TIERS_SECOND_LINE_Y, 0.7F, GuiQuestBook.WEIGHT_SPACING, 0, 0x404040);
			}
		}
		...
		for (int i = start; i < Math.min(start + GuiQuestBook.VISIBLE_GROUPS, groups.size()); i++) {
			...
			boolean inBounds = gui.inBounds(GuiQuestBook.GROUPS_X, yPos, gui.getStringWidth(str),
					GuiQuestBook.TEXT_HEIGHT, x, y);
			...
			gui.drawString(Translator.translate("hqm.questBook.items", group.getItems().size()),
					GuiQuestBook.GROUPS_X + GuiQuestBook.GROUPS_SECOND_LINE_X, yPos + GuiQuestBook.GROUPS_SECOND_LINE_Y,
					0.7F, 0x404040);
		}
	}
	...
	public void draw(GuiQuestBook gui, int x, int y) {
		gui.drawString(this.getName(), GuiQuestBook.GROUPS_X, GuiQuestBook.GROUPS_Y,
				this.getTier().getColor().getHexColor());
		...
		for (int i = 0; i < items.size(); i++) {
			...
			if (gui.inBounds(xPos, yPos, GuiBase.ITEM_SIZE, GuiBase.ITEM_SIZE, x, y)) {
				if (itemStack != null && itemStack.getItem() != null) {
					try {
						gui.drawMouseOver(
								itemStack.getTooltip(Minecraft.getMinecraft().thePlayer,
										Minecraft.getMinecraft().gameSettings.advancedItemTooltips),
								x + gui.getLeft(), y + gui.getTop());
					}
		...
		for (int i = 0; i < Math.min(DataBitHelper.GROUP_ITEMS.getMaximum(), items.size() + 1); i++) {
			...
			if (gui.inBounds(xPos, yPos, GuiBase.ITEM_SIZE, GuiBase.ITEM_SIZE, x, y)) {
				if (gui.getCurrentMode() == EditMode.ITEM) {
					...
					gui.setEditMenu(new GuiEditMenuItem(gui, gui.getPlayer(), itemStack, i,
							GuiEditMenuItem.Type.BAG_ITEM, amount, ItemPrecision.PRECISE));
				}
	...
	public static void mouseClickedOverview(GuiQuestBook gui, ScrollBar groupScroll, int x, int y) {
		...
		for (int i = start; i < Math.min(start + GuiQuestBook.VISIBLE_GROUPS, groups.size()); i++) {
			...
			if (gui.inBounds(GuiQuestBook.GROUPS_X, posY, gui.getStringWidth(group.getName()), GuiQuestBook.TEXT_HEIGHT,
					x, y)) {
				switch (gui.getCurrentMode()) {
					...
					case NORMAL:
						...
						gui.getTextBoxGroupAmount().setTextAndCursor(gui,
								String.valueOf(GuiQuestBook.getSelectedGroup().getLimit()));
						break;
					case RENAME:
