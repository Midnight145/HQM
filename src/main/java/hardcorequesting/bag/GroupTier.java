...
public class GroupTier {
	...
	public String getName() { return name == null || name.equals("") ? Translator.translate("hqm.bag.unknown") : name; }

	public GuiColor getColor() { return color; }

	public int[] getWeights() { return weights; }

	public static List<GroupTier> getTiers() { return QuestLine.getActiveQuestLine().tiers; }

	public void setName(String name) { this.name = name; }

	public GroupTier copy() { return new GroupTier(getName(), getColor(), getWeights()); }
	...
	public static void mouseClickedOverview(GuiQuestBook gui, ScrollBar tierScroll, int x, int y) {
		...
		for (int i = start; i < Math.min(start + GuiQuestBook.VISIBLE_TIERS, tiers.size()); i++) {
			...
			if (gui.inBounds(GuiQuestBook.TIERS_X, posY, gui.getStringWidth(groupTier.getName()),
