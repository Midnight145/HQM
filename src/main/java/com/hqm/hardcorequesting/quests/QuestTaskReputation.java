...
import com.hqm.hardcorequesting.client.interfaces.GuiEditMenuReputationSetting;
...
public abstract class QuestTaskReputation extends QuestTask {
    ...
    public static class ReputationSetting {
        ...
        public boolean isValid(String playerName) {
            if (this.getReputation() == null || !this.getReputation()
                .isValid()) {
                return false;
            }
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i >= this.settings.length) {
                gui.drawRect(
                    ...
                    START_Y + this.startOffsetY + i * OFFSET_Y + Reputation.BAR_Y,
                ...
                info = setting.reputation.draw(
                    gui,
                    START_X,
                    START_Y + this.startOffsetY + i * OFFSET_Y,
                    mX,
                    mY,
                    info,
                    this.getPlayerForRender(player),
                    true,
                    setting.lower,
                    setting.upper,
                    setting.inverted,
    ...
    public void onClick(GuiQuestBook gui, EntityPlayer player, int mX, int mY, int b) {
        if (Quest.isEditing && gui.getCurrentMode() != EditMode.NORMAL) {
            ...
            for (int i = 0; i < size; i++) {
                if (gui
                    .inBounds(START_X, START_Y + this.startOffsetY + i * OFFSET_Y, Reputation.BAR_WIDTH, 20, mX, mY)) {
    ...
    public void autoComplete(String playerName) {
        this.getData(playerName).completed = true;
    }
}
